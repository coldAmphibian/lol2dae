...
BEGIN_MESSAGE_MAP(Clol2daeDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTONINPUT, &Clol2daeDlg::OnBnClickedButtoninput)
...
BOOL Clol2daeDlg::OnInitDialog()
{
	...
	m_EditCtrlBrush = CreateSolidBrush(RGB(96, 96, 96));
...
void Clol2daeDlg::OnBnClickedButtoninput()
{
	CStringW fileName;
	...
	ofn.lpstrFilter = L"League of Legends SKN (*.skn)\0*.skn\0All Files (*.*)\0*.*\0";
	ofn.lpstrDefExt = L"skn";

	if (openFileDlg.DoModal() == IDOK)
	{
		m_Input.SetWindowTextW(fileName);
...
UINT Clol2daeDlg::convertWorkerThread(LPVOID lParam)
{
	...
	CStringW sknPath;
	dlg->m_Input.GetWindowTextW(sknPath);
	...
	try
	{
		...
		ColladaWriter outputCollada(inputSkn.indices, inputSkn.vertices, inputSkl.bones, inputSkl.boneIndices);
