...
bool SklImporter::readFile(TCHAR* path)
{
	...
	if (fileVersion == 1953262451)
	{
		...
		{
			inFile.read(reinterpret_cast<char*>(&bones[i].name), 32);
			inFile.read(reinterpret_cast<char*>(&bones[i].parent), 4);
			inFile.read(reinterpret_cast<char*>(&bones[i].scale), 4);
			inFile.read(reinterpret_cast<char*>(&bones[i].globalMatrix), 48);
			bones[i].globalMatrix[12] = 0;
			bones[i].globalMatrix[13] = 0;
			bones[i].globalMatrix[14] = 0;
			bones[i].globalMatrix[15] = 1;
		}
		...
		else if (*sknVersion == 2)
		{
			...
			for (int i = 0; i < numBoneIndices; i++)
			{
				inFile.read(reinterpret_cast<char*>(&boneIndices[i]), 4);
			}
		}

		for (int i = 0; i < numBones; i++)
		{
			invertMatrix(bones[i].globalMatrix, bones[i].inverseMatrix);
		}
		...
		for (int i = 0; i < numBones; i++)
		{
			if (bones[i].parent != -1)
			{
				multiplyMatrix(bones[bones[i].parent].inverseMatrix, bones[i].globalMatrix, bones[i].relativeMatrix);
			}
		}
	}
	...
	else if (fileVersion == 587026371)
	{
		...
		for (int i = 0; i < numBones; i++)
		{
			...
			quaternionToMatrix(x, y, z, w, bones[i].relativeMatrix);
			bones[i].relativeMatrix[3] = tx;
			bones[i].relativeMatrix[7] = ty;
			bones[i].relativeMatrix[11] = tz;
		...
		for (int i = 0; i < numBoneIndices; i++)
		{
			inFile.read(reinterpret_cast<char*>(&boneIndices[i]), 2);
			boneIndices[i] = short(boneIndices[i]);
		}
		...
		for (int i = 0; i < numBones; i++)
		{
			...
			while (!foundEnd)
			{
				...
				{ 
					...
					{
						bones[i].name[charIndex] = currentChar;
						charIndex++;
					}
				}
			}
		...
		for (int i = 0; i < numBones; i++)
		{
			if (bones[i].parent != -1)
			{
				multiplyMatrix(bones[bones[i].parent].globalMatrix, bones[i].relativeMatrix, bones[i].globalMatrix);
			}
			...
			else
			{
				for (int j = 0; j < 16; j++)
				{
					bones[i].globalMatrix[j] = bones[i].relativeMatrix[j];
				}
			}
		}
